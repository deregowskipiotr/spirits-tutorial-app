...
const vodkaTypes = ["Premium", "Standard", "Flavored", "Unique"];
        ...
        >
          <div className="flex items-center gap-6 mb-8">
            <div className="w-20 h-20 rounded-full border border-brand-accent/20 flex items-center justify-center">
              <GiGlassShot className="text-4xl text-brand-accent" />
            </div>
            <div>
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-light tracking-tight">VODKA</h1>
              <p className="text-brand-accent/60 text-sm uppercase tracking-[0.2em] mt-2">Crystalline Clarity</p>
            </div>
          </div>
          ...
          <p className="text-gray-400 text-xl font-extralight max-w-3xl leading-relaxed italic">
            The pursuit of purity. Discover the subtle nuances of grain and potato distillates, 
            where every filter and distillation step defines the soulful character of the spirit.
          </p>
        </motion.div>

        {/* Placeholder for Products */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-12">
          {vodkaTypes.map((type, index) => {            
            return (
              <motion.div
                key={type}
              ...
              >
                <Link href={`/spirits/vodka/${type.toLowerCase()}`}>
                  <div 
