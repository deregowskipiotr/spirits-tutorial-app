...
const rumTypes = ["Premium", "White", "Dark", "Spiced"];
        ...
        >
          <div className="flex items-center gap-6 mb-8">
            <div className="w-20 h-20 rounded-full border border-brand-accent/20 flex items-center justify-center">
              <GiSnowBottle className="text-4xl text-brand-accent" />
            </div>
            <div>
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-light tracking-tight">RUM</h1>
              <p className="text-brand-accent/60 text-sm uppercase tracking-[0.2em] mt-2">Caribbean Heritage</p>
            </div>
          </div>
          ...
          <p className="text-gray-400 text-xl font-extralight max-w-3xl leading-relaxed italic">
            The alchemy of sugar and sun. Explore the diverse spectrum from crisp white rums 
            to the deep, mahogany depths of long-aged Caribbean treasures.
          </p>
        </motion.div>

        {/* Placeholder for Products */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-12">
          {rumTypes.map((type, index) => {            
            return (
              <motion.div
                key={type}
              ...
              >
                <Link href={`/spirits/rum/${type.toLowerCase()}`}>
                  <div 
