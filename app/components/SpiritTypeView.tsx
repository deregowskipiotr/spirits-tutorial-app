...
export default function SpiritTypeView({ data, categoryName, categoryPath }: SpiritTypeViewProps) {
  return (
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        ...
        <div className="space-y-32 lg:space-y-48">
          {data.sections.map((section, index) => (
            ...
            >
              <div className={`${section.imagePosition === 'left' ? 'order-1 lg:order-1' : 'order-2 lg:order-2'} h-full`}>
                <ImageWithPlaceholder src={section.image} alt={`${data.title} - Section ${section.id}`} />
              </div>
              <div className={`${section.imagePosition === 'left' ? 'order-2 lg:order-2' : 'order-1 lg:order-1'}`}>
        ...
        >
          <Link href={categoryPath}>
            ...
            >
              <FiArrowLeft className="text-sm" /> Back to {categoryName} Products
            </button>
          </Link>
        </motion.div>
